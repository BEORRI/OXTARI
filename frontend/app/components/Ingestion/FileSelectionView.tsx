...
const FileSelectionView: React.FC<FileSelectionViewProps> = ({
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (files.length > 0) {
      // Create a synthetic event to match the handleUploadFiles signature
      ...
      await handleUploadFiles(syntheticEvent, false);
      addStatusMessage(`Dropped ${files.length} file(s)`, "SUCCESS");
    }
  };
  ...
  const handleUploadFiles = async (
    ...
    if (event.target.files && RAGConfig) {
      ...
      // File size limits (in bytes) - Optimized for better performance
      const MAX_FILE_SIZE = 100 * 1024 * 1024; // 100MB - Increased for better performance
      const MAX_TOTAL_SIZE = 500 * 1024 * 1024; // 500MB for multiple files - Increased for better throughput
      ...
      if (oversizedFiles.length > 0) {
        addStatusMessage(
          `Files too large (max 100MB): ${oversizedFiles.join(", ")}`,
          "ERROR"
        );
        return;
      }
      ...
      if (totalSize > MAX_TOTAL_SIZE) {
        addStatusMessage(
          `Total file size too large (max 500MB). Current: ${(totalSize / 1024 / 1024).toFixed(1)}MB`,
          "ERROR"
        );
        return;
      }
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="bg-bg-alt-oxtari rounded-2xl flex gap-2 p-3 items-center justify-end lg:justify-between h-min w-full shadow-lg border border-gray-200">
        ...
        <div className="flex gap-3 justify-center lg:justify-end">
          ...
          <div className="dropdown dropdown-hover">
            <label tabIndex={0}>
              <BEORRIButton title="Directory" Icon={GoFileDirectoryFill} text_size="text-lg" icon_size={20} />
            </label>
            <ul
              tabIndex={0}
              className="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow"
          ...
          <div className="dropdown dropdown-hover">
            <label tabIndex={0}>
              <BEORRIButton title="URL" Icon={IoMdAddCircle} text_size="text-lg" icon_size={20} />
            </label>
      ...
      <div 
        className={`bg-bg-alt-oxtari rounded-2xl flex flex-col gap-3 p-6 items-center justify-start h-full w-full overflow-auto relative transition-all duration-200 shadow-lg border border-gray-200 ${
      ...
      >
        {isDragging && (
          <div className="absolute inset-0 flex items-center justify-center bg-blue-100/90 rounded-2xl z-10 pointer-events-none">
        ...
        {Object.entries(fileMap).length === 0 && !isDragging && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-3">
            <div className="text-6xl">📂</div>
            <p className="text-lg font-semibold">No files selected</p>
            <p className="text-sm">Drag & drop files here or click the buttons above</p>
          </div>
        )}
      ...
      {/* Import Footer */}
      {socketStatus === "ONLINE" ? (
        <div className="bg-bg-alt-oxtari rounded-2xl flex gap-2 p-3 items-center justify-end h-min w-full shadow-lg border border-gray-200">
