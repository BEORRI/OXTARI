...
const SettingsComponent: React.FC<SettingsComponentProps> = ({
  ...
  const renderSettingComponent = (
    ...
    return (
      <div key={title}>
        <div className="flex gap-3 justify-between items-center text-gray-900">
          <p className="flex min-w-[8vw] font-medium">{setting_type.description}</p>
          ...
          {setting_type.type === "select" && (
            <select
              ...
              className="select bg-white border-2 border-gray-200 hover:border-gray-400 focus:border-gray-400 focus:outline-none transition-all duration-200 text-gray-700 font-medium rounded-lg shadow-sm hover:shadow-md cursor-pointer"
            >
              {setting_type.options.map((template) => (
                <option key={"Select_" + template} value={template} className="py-2">
                  {template}
                </option>
              ))}
            </select>
          ...
          {setting_type.type === "color" && (
            <div className="flex flex-col gap-1 h-[15vh] z-10">
              <label className="input bg-bg-oxtari input-sm input-bordered flex items-center gap-2 w-full">
                <input
                  type="text"
                  className="grow"
                  placeholder={title}
