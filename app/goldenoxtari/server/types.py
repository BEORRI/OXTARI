...
class FileConfig(BaseModel):
    ...
    def validate_file_size(cls, v):
        MAX_FILE_SIZE = 100 * 1024 * 1024  # 100MB - Updated to match frontend optimization
        if v > MAX_FILE_SIZE:
            raise ValueError(f'File size {v} bytes exceeds maximum allowed size of {MAX_FILE_SIZE} bytes (100MB)')
        return v
