...
from goldenoxtari.components.managers import (
    ReaderManager,
    ChunkerManager,
    EmbeddingManager,
    RetrieverManager,
...
class OxtariManager:
    ...
    async def import_document(
        ...
        try:
            ...
            # Ensure Weaviate client is healthy before starting import
            await self.weaviate_manager.ensure_connection_healthy(client)
            msg.info(f"Starting import process for {fileConfig.filename} with healthy Weaviate connection")
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    msg.fail(f"Document {i+1} processing failed: {str(result)}")
            ...
            error_msg = str(e)
            msg.fail(f"Import failed for {fileConfig.filename}: {error_msg}")
            await logger.send_report(
                fileConfig.fileID,
                status=FileStatus.ERROR,
                message=f"Import for {fileConfig.filename} failed: {error_msg}",
                took=0,
            )
            return
    ...
    async def process_single_document(
        ...
        # Ensure Weaviate client is healthy at the start
        await self.weaviate_manager.ensure_connection_healthy(client)
        msg.info(f"Processing document: {document.title}")
        ...
        try:
            # Check and reconnect Weaviate client if needed
            try:
                if not await client.is_ready():
                    msg.warn("Weaviate client not ready, reconnecting...")
                    await client.connect()
            except Exception as e:
                msg.warn(f"Weaviate client connection lost, reconnecting: {e}")
                await client.connect()
            ...
            # Ensure Weaviate client is healthy after long embedding process
            await self.weaviate_manager.ensure_connection_healthy(client)
            msg.info(f"Starting Weaviate import for {len(vectorized_documents)} documents")

            # Process documents sequentially to avoid overwhelming the connection
            for i, document in enumerate(vectorized_documents):
                try:
                    # Check connection health before each document import
                    await self.weaviate_manager.ensure_connection_healthy(client)
                    
                    msg.info(f"Importing document {i+1}/{len(vectorized_documents)}: {document.title}")
                    
                    # Use the enhanced import_document method with retry mechanism
                    ...
                    msg.good(f"Successfully imported document {i+1}/{len(vectorized_documents)}")
                    
                except Exception as e:
                    msg.fail(f"Failed to import document {i+1}: {str(e)}")
...
class ClientManager:
    def __init__(self) -> None:
        ...
        self.manager: OxtariManager = OxtariManager()
    ...
    async def connect(
        ...
        async with lock:
            if cred_hash in self.clients:
                ...
                now = datetime.now()
                last_check = self.last_health_check.get(cred_hash, datetime.min)
                time_since_check = (now - last_check).total_seconds()
                
                if time_since_check > self.connection_health_check_interval:
                    msg.info("Performing periodic health check on existing client")
                    try:
                        # Perform health check
                        await self.manager.ensure_connection_healthy(self.clients[cred_hash]["client"])
                        self.last_health_check[cred_hash] = now
                        msg.info("Client health check passed")
                    except Exception as e:
                        msg.warn(f"Client health check failed, reconnecting: {e}")
                        # Remove the unhealthy client and create a new one
                        try:
                        ...
                        msg.info("Found existing healthy Client")
                        return self.clients[cred_hash]["client"]
                else:
                    msg.info("Found existing Client (health check not needed)")
                    return self.clients[cred_hash]["client"]
            ...
            try:
                ...
                if client:
                    ...
                    self.last_health_check[cred_hash] = datetime.now()
                    return client
                else:
