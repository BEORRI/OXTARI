...
// Cache the detected host to avoid repeated API calls
let cachedHost: string | null = null;

// Function to reset the cached host (useful for testing or when backend changes)
export const resetHostCache = (): void => {
  cachedHost = null;
};
...
export const detectHost = async (): Promise<string> => {
  ...
  // In development, prioritize localhost backend
  ...
  // Try the current origin (for remote access)
...
export const connectToBEORRI = async (
  ...
  try {
    ...
    console.error("Error connecting to BEORRI:", error);
    return {
      connected: false,
